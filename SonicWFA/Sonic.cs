...
namespace SonicWFA
{
    public class Sonic
    {
        ...
        public void ImportarCanciones(string nombre, string cantante, string genero, string album, string estudio, string discografia, string compositor, int añoPublicacion, int duracionMinutos, int duracionSegundos, string archivoMP3, string rutaArchivo) //Metadata de Importacion
        {
                ...
                Compositor compositor2 = AgregarCompositor(compositor);
                int duracion = (duracionMinutos * 60 + duracionSegundos);
                Cancion cancion = new Cancion(nombre, cantante2, album2,  genero, estudio, discografia, compositor2, añoPublicacion, duracion, archivoMP3, rutaArchivo);
                canciones.Add(cancion);
                this.AgregarCancionCantante(cancion, cantante);
                this.AgregarCancionCompositor(cancion, compositor);
                this.AgregarCancionAlbum(cancion, album);
        ...
        public void ImportarVideos(List<Actor> actores1, string titulo, string categoria, string genero, string estudio, string director, string descripcion, int añoDePublicacion, int resolucion, int duracionMinutos, int duracionSegundos, string archivoMP3, string rutaAcrhivo) //Metadata de importacion
        {
           
                Director director1 = AgregarDirector(director);
                int duracion = (duracionMinutos * 60 + duracionSegundos);
                Video video = new Video(titulo, categoria, genero, estudio, director1, descripcion, actores1, añoDePublicacion, duracion, resolucion, archivoMP3, rutaAcrhivo);
                videos.Add(video);
                foreach(Actor actor in actores1) { AgregarVideoActor(video, actor.nombre); }
                this.AgregarVideoDirector(video, director);
...
=======
        ...
        public List<Cancion> BuscarCancion(string palabra = "qwery", string persona = "qwery", int eleccion3 = 0, int valor = 0, int eleccion7 = 0, int valor2 = 0, string categoria = "qwery")
        {
            string resultado = "";
            ...
            List<Cancion> canciones = new List<Cancion>(cancionesBusqueda);
            //if (videosBusqueda.Count == 0 && cancionesBusqueda.Count == 0) { return "NO HAY RESULTADOS"; }
            cancionesBusqueda.Clear();
            videosBusqueda.Clear();
            return canciones;
        ...
        public List<Video> BuscarVideo(string palabra = "qwery", string persona = "qwery", int eleccion3 = 0, int valor = 0, int eleccion7 = 0, int valor2 = 0, string categoria = "qwery")
        {
            string resultado = "";
            ...
            List<Video> videos = new List<Video>(videosBusqueda);
            //if (videosBusqueda.Count == 0 && cancionesBusqueda.Count == 0) { return "NO HAY RESULTADOS"; }
            cancionesBusqueda.Clear();
            videosBusqueda.Clear();
            return videos;
        ...
        public void AgregarCancionAFavoritos(Usuario2 usuario, Cancion cancion)
        {
